// To decompress the compressed text file use below commands in linux terminal:
// $ gcc puff.c
// $ ./a.out [uncompressed_filename].txt [compressed_filename].bin [output_filename].txt
// author : Arpit Sharma
...
int main(int argc, char* argv[])
{
	...
	// iterating over all the characters and hashing them in the table
	while(ch = fgetc(ifp))
	{
		eof = ch;
		if (eof == EOF)
